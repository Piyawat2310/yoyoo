...
def fetch_gold_api_data(**context):
    try:
        api_key = 'goldapi-3vwscsm6ka6scc-io'
        headers = {
        ...
        logger.info(f"Starting monthly data fetch for execution_date: {execution_date}, inc_day: {inc_day}")

        # สร้างลิสต์ของวันที่ต้องการดึงข้อมูล (30 วัน)
        target_dates = []
        for i in range(30):
...
dag = DAG(
    'etl_goldapi_monthly_historical',  # เปลี่ยนชื่อ DAG
    default_args=default_args,
    description='One-time DAG for fetching 30 days of historical gold price data',
    schedule_interval=None,  # กำหนดเป็น None เพื่อรันครั้งเดียว
    catchup=False
)
